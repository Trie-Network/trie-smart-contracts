...
func main() {
	...
	r.POST("/api/onboard-infra", handleOnboardInfra)
	r.GET("/api/provider-info", store.GetProviderInfo)
...
func handleOnboardInfra(c *gin.Context) {
	nodeAddress := "http://localhost:20009"
	
	selfContractHash := os.Getenv("SELF_CONTRACT_HASH")
	if selfContractHash == "" {
		wrapError(c.JSON, "unable to fetch selfContractHash")
		return
	}

	selfContractHashPath := path.Join("../artifact/onboarding_contract.wasm")
	...
	smartContractInfo, err := getSmartContractInfo(nodeAddress, selfContractHash)
	if err != nil {
		wrapError(c.JSON, fmt.Sprintf("unable to get smart contract blocks, err: %v", err))
		return
	}

	smartContractContractData, err := getSmartContractData(smartContractInfo)
	if err != nil {
		wrapError(c.JSON, fmt.Sprintf("unable to get smart contract data, err: %v", err))
		return
	}

	// Create Import function registry
	hostFnRegistry := wasmbridge.NewHostFunctionRegistry()
	hostFnRegistry.Register(NewVerifyAction())
	...
	res, err := wasmModule.CallFunction(smartContractContractData)
	if err != nil {
		wrapError(c.JSON, fmt.Sprintf("unable to execute function, err: %v", err))
		return
	}

	msg, errMsg := extractSignatureVerificationOutput(res)
	if errMsg != "" {
